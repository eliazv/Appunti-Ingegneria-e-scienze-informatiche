...
# creo tutte le pagine del server
...
def create_index_page():
    ...
    try:
        ...
        message = message + '<th colspan="4"><h3><ul><li><a href="/Relazione_Zavatta.pdf" download="Relazione_Zavatta.pdf">Download file pdf</a></li></ul></h3></th>'
...
def signal_handler(signal, frame):
    print( 'Exiting the http server... (Ctrl+C pressed)')
    try:
      if(server):
        server.server_close()
    finally:
...
def main():
    #creo i contenuti del server
    resfresh_contents()
    #usando la combinazione di tasti Ctrl+C terma tutti i thread generati
    ...
    #interrompe l'esecuzione se si premono da tastiera i tasti CTRL + C 
    signal.signal(signal.SIGINT, signal_handler)
    # cancella i dati get del documento che ne tiene traccia ogni volta che il server viene avviato
